...
<div class="ranking" class:pink={accent === 'pink'}>
	{#each rankings as dancer}
		<a class="row" href={`/dancers/${dancer.slug}`}><strong>{dancer.rank}</strong><span>{dancer.stageName}<small>{dancer.countryCode ?? '—'}</small></span><b>{dancer.points.toLocaleString()} pts</b></a>
	{/each}
...
<style>
	...
	small { margin-left: 9px; color: #838d91; font-size: .61rem; }
