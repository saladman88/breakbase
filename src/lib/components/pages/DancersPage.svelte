			...
			<a class={`card ${tones[index % tones.length]}`} href={`/dancers/${dancer.slug}`}>
				...
				<div class:photo={dancer.slug === 'hong-10'} class="avatar">
					{#if dancer.slug === 'hong-10'}
						<img src="/images/dancers/hong-10/portrait-cutout.webp" alt="Hong 10 performing a freeze" loading="lazy" decoding="async" />
					{:else}
						<b>{dancer.stageName.slice(0, 2).toUpperCase()}</b>
					{/if}
				</div>
				<div class="copy"><small><CountryFlag code={dancer.countryCode} country={dancer.country} size="sm" /> {dancer.country}</small><h2>{dancer.stageName}</h2><p>{dancer.crew ?? 'Independent'}</p></div>
...
<style>
	...
	.avatar.photo { width: 210px; aspect-ratio: 1.25; border-radius: 0; background: radial-gradient(circle, currentColor 0, transparent 68%); transform: translate(-36px, -30px); }
	.avatar.photo img { width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 12px 14px rgba(0,0,0,.45)); }
	.copy small { display: flex; align-items: center; gap: 7px; }
